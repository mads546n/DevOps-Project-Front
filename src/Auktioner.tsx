...
function Auktioner() {
    ...
    const [artist, setArtist] = useState<ArtItem[]>(data.items);

    const filterFromTag = (tag: string) => {
        console.log(`Filter auctions by tag: ${tag}`);
        setArtist(data.items.filter((item) => item.tag === tag));
    };
  ...
  return (
    <>
        ...
        <button onClick={() => setArtist(data.items)}>Nulstil filter</button>
        <button onClick={() => filterFromTag("Litografi")}>Litografi</button>
        <button onClick={() => filterFromTag("Akvarel")}>Akvarel</button>
        <button onClick={() => filterFromTag("Olie på lærred")}>Olie på lærred</button>
        <button onClick={() => filterFromTag("Grafik")}>Grafik</button>
        <button onClick={() => filterFromTag("Collage")}>Collage</button>
        <button onClick={() => filterFromTag("Tryk")}>Tryk</button>
        <button onClick={() => filterFromTag("Akryl")}>Akryl</button>

        <ArtistCarousel data={{ ...data, items: artist }} />
